...
class CustomerMainComponent extends Component {
  ...
  componentDidMount() {
    let customer = this.props.customers.customers.filter(
      (customer) => customer.Customer_Username === this.props.auth.user.username
    )[0];

    let serviceProviders_by_city = this.props.serviceProviders.serviceproviders;

    let serviceProviders_by_city_added_feedbacks = [];
    if (serviceProviders_by_city != null) {
      serviceProviders_by_city.forEach((element) => {
        let feedbacks = this.props.feedbacks.feedbacks.filter(
          (fb) =>
            fb.ServiceProvider_Username === element.ServiceProvider_Username
        );
        let pos = 0;
        let neg = 0;
    ...
    let plumbers_by_city = serviceProviders_by_city_added_feedbacks.filter(
      (plumber) => plumber.Occupation === "Plumber"
    );
    const plumbers_list = this.state.Plumbers.concat(plumbers_by_city);

    let carpenters_by_city = serviceProviders_by_city_added_feedbacks.filter(
      (carpenter) => carpenter.Occupation === "Carpenter"
    );
    const carpenters_list = this.state.Carpenters.concat(carpenters_by_city);

    let electricians_by_city = serviceProviders_by_city_added_feedbacks.filter(
      (electricians) => electricians.Occupation === "Electrician"
    );
    const electricians_list =
      this.state.Electricians.concat(electricians_by_city);

    let barbers_by_city = serviceProviders_by_city_added_feedbacks.filter(
      (barber) => barber.Occupation === "Barber"
    );
    const barbers_list = this.state.Barbers.concat(barbers_by_city);

    let applianceRepair_by_city =
      serviceProviders_by_city_added_feedbacks.filter(
        (applianceRepair) => applianceRepair.Occupation === "Appliance Repair"
      );
    const applianceRepair_list = this.state.ApplianceRepair.concat(
      applianceRepair_by_city
    );

    let acServiceRepair_by_city =
      serviceProviders_by_city_added_feedbacks.filter(
        (acServiceRepair) => acServiceRepair.Occupation === "AC Service/Repair"
      );
    const acServiceRepair_list = this.state.ACServiceRepair.concat(
      acServiceRepair_by_city
    );

    let booking = [];
    let ct_booking = this.props.bookings.bookings.filter(
      (ct) => ct.Customers_Username === this.props.auth.user.username
    );
      ...
      ct_booking.forEach((element) => {
        booking.push({
          ...
          Action: (
            <div>
              <i
                className="fa fa-check-circle resolve mr-2"
                onClick={() => {
                  if (element.Feedback === false) {
                    alert("Please Provide Feedback");
                  } else if (
                    window.confirm(
                      "Are you sure Your Booked Service is Completed"
                    )
                  ) {
                    let booking_update = {
                      id: element._id,
                      ...
                      ServiceProviders_Username:
                        element.ServiceProviders_Username,
                      DateTime: element.DateTime,
                      ProblemDescription: element.ProblemDescription,
                      TimeSlot: element.TimeSlot,
                      Completed: true,
              ...
              <Link to={`/Customer/Feedback/${element._id}`}>
                <i class="fas feed mr-2 fa-clipboard-list"></i>
              </Link>
    ...
    let completed_bookings = this.props.bookings.bookings.filter(
      (ct) => ct.Customers_Username === this.props.auth.user.username
    );
  ...
  render() {
    const Booking = ({ match }) => {
      return (
        <BookingForm
          Service_provider_username={match.params.username}
          Customer_username={this.state.customer.Customer_Username}
        ...
        <FeedbackForm
          booking_details={
            this.state.bookings.filter(
              (el) => el.id === match.params.username
            )[0]
          }
          Customer_username={this.state.customer.Customer_Username}
        ...
        <ServiceProviderProfile
          serviceProvider={
            this.props.serviceProviders.serviceproviders.filter(
              (sp) => sp.ServiceProvider_Username == service_provider
            )[0]
          }
          feedbacks={this.props.feedbacks.feedbacks.filter(
            (fb) => fb.ServiceProvider_Username === service_provider
          )}
        />
      );
    };
        ...
        <div className="row w-100">
          ...
          <div className="col-md-9">
            <Switch>
              ...
              <Route
                ...
                component={() => (
                  <CustomerUpdateProfile
                    customer={this.state.customer}
                    updateCustomerInfo={this.props.updateCustomerInfo}
                  />
                )}
              />
              <Route path="/Customer/services" component={() => <Services />} />
              <Route
                path="/Customer/Plumbers"
                component={() => (
                  <CustomerPlumbers
                    plumbers={this.state.Plumbers.sort(
              ...
              />
              <Route
                path="/Customer/AC ServiceRepair"
                component={() => (
                  <CustomerAC
                    ACServiceRepair={this.state.ACServiceRepair.sort(
                      (a, b) => b.Feedback_count - a.Feedback_count
                    )}
                  />
                )}
              />
              <Route
                path="/Customer/Appliance Repair"
                component={() => (
                  <CustomerAppliance
                    ApplianceRepair={this.state.ApplianceRepair.sort(
                      (a, b) => b.Feedback_count - a.Feedback_count
                    )}
                  />
                )}
              />
              <Route
                path="/Customer/Electricians"
                component={() => (
                  <CustomerElectrician
                    Electricians={this.state.Electricians.sort(
                      (a, b) => b.Feedback_count - a.Feedback_count
                    )}
                  />
                )}
              />
              <Route
                path="/Customer/Carpenters"
                component={() => (
                  <CustomerCarpenter
                    Carpenters={this.state.Carpenters.sort(
                      (a, b) => b.Feedback_count - a.Feedback_count
                    )}
                  />
                )}
              ...
              <Route
                ...
                component={() => (
                  <CustomerBarber
                    Barbers={this.state.Barbers.sort(
                      (a, b) => b.Feedback_count - a.Feedback_count
                    )}
                  />
                )}
              ...
              <Route
                ...
                component={() => (
                  <CustomerBookings bookings={this.state.bookings} />
                )}
              />
              <Route
                path="/Customer/AvailServices"
                component={() => (
                  <CustomerBookedServices
                    bookings={this.state.completed_booking}
                  />
                )}
              />
              <Route
                path="/Customer/:username"
                component={ServiceProviderProfil}
              />
