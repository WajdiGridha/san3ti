...
function LoginForm(props) {
  ...
  useEffect(() => {
    if (props.auth.isAuthenticated && props.auth.admin)
      history.push("/ServiceProvider/Profile");
    else if (props.auth.isAuthenticated && !props.auth.admin)
            ...
            <CardBody>
              <Form onSubmit={handleLoginClick}>
                ...
                <button
                  type="submit"
                  value="submit"
                  className="btn btn-primary btn-md btn-block mt-2 color"
                >
                  Login
                </button>
              </Form>
              {props.auth.errMess && (
                <div>
                  <p>{props.auth.errMess} Please try again</p>
                </div>
              )}
            </CardBody>
          }
